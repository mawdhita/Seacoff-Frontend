...
const Home = () => {
  ...
  const fetchMenu = async () => {
    try {
      ...
      setCoffees(menuData);
      // Filter out any empty or falsy categories to avoid empty buttons
      const uniqueCategories = ['All', ...new Set(menuData.map(item => item.kategori).filter(Boolean))];
